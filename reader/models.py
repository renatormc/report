...
user_group = Table('user_group', Base.metadata,
                      Column('group', Integer, ForeignKey(
                          'group.id'), nullable=False),
                      Column('user', Integer, ForeignKey(
...
class User(Base):
    ...
    def set_password(self, pw):
        self.pw_hash = pbkdf2_sha256.hash(pw)
     
    def check_password(self, pw):
        if pbkdf2_sha256.verify(pw, self.pw_hash):
            return True
        else:
            return False
...
class Chat(Base):
    ...
    friendly_identifier = Column(String(300))
...
class File(Base):
    ...
    converted_path = Column(String(500))
    ...
    @hybrid_property
    def has_thumb(self):
        return True if self.analise_thumb  else False
    ...
    @hybrid_property
    def link(self):
        return quote(self.path.replace("\\", "/"))
